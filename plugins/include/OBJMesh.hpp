...
private:
    ...
    std::string resolveTexturePath(const std::string &texPath,
        bool mustExist = false) const;
    std::string findFallbackDiffuseTexture(
    ...
    std::string tryExtensionVariants(const std::filesystem::path &filename,
                        const std::vector<std::filesystem::path> &roots) const;
    std::vector<std::string> buildMaterialNameVariants(
